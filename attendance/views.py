...
def detail(request, class_id):
    cur = connection.cursor()
    cur.execute(f'SELECT id, class_date, question_of_the_day FROM classes WHERE id = %s;', [class_id])
...
def store(request):
    ...
    return HttpResponseRedirect(reverse("attendance:create"))
...
from .models import MultipleChoiceQuestion, OpenQuestion


def quiz(request):
    mc_questions = MultipleChoiceQuestion.objects.all()
    open_questions = OpenQuestion.objects.all()
    return render(request, 'quiz.html', {'mc_questions': mc_questions, 'open_questions': open_questions})